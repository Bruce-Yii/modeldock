// Guaranteed node_repl MCP tools (Computer Use / browser automation entry point).
// These schemas are captured verbatim from Codex's own tool list; they are injected by
// the transform whenever the session payload does not already carry them, so the model
// always has a working JavaScript + screen-control entry point even when Codex's
// lazy-loaded MCP tools are missing from the session (e.g. after a node_repl restart).
export const NODE_REPL_MCP_TOOLS = [
  {
    "type": "function",
