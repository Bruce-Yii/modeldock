...
3. **Never let image-gen carry product identity.** Generative image models
   ...
   layouts already validated by a human eye, (d) plan-stage outline /
   storyboard thumbnails, and (e) transparent-background decorative sprites
   (smoke, embers, glow orbs, sparkles, dust, leaves, generic non-brand
   icons) for animation layers. (d) is a planning reference, never a final frame:
   it locks composition, camera, and mood early so the script and scenes are
   built against a seen plan, but product identity still comes from labeled
   real UI + 3D forms + text at production time. Sprites (e) are composited
   and animated with transforms (x/y/scale/rotation) only; they never carry
   identity either.
...
## 3. Stage 0 - Concept, outline, and storyboard
...
2. Design the outline and storyboard with image-gen BEFORE writing final
   copy. Generate one panel per planned shot (or 2-3 style anchors when shots
   share a look) showing composition, camera direction, palette/mood, and
   where identity text or real UI will sit. Panels are planning references
   only - never promote them to final frames; product identity still comes
   from labeled real UI, 3D forms, and text at production (principle 3).
   On every panel, also write the shot's tech stack (primary technique +
   layers): three.js core / HTML+GSAP kinetic type / image-gen sprite /
   real-UI shot / data-viz / Lottie (see references/tech-stack.md).
3. Use the storyboards to validate the outline: does the sequence tell the
   one-line message? Is every claim backed by a visible proof point? Fix the
   flow here, where a panel costs one generation instead of a scene rebuild.
4. Write narration line-by-line (one line per shot) into a JSON array of
   { text, dur }. Durations are filled later from measured TTS, never guessed.
5. Keep the story to what the product actually shows. Every claim needs a
   visual or a real UI shot to back it.

### Plan-stage image-gen prompts that work

- Storyboard panel: "1920x1080 storyboard frame for a <genre> promo, shot <n>:
  <subject> entering from <direction>, slow push-in, deep navy stage with
  cyan/emerald rim light, marked zones for text labels and real UI, film grid
  lines with the shot number, no readable text, no logos."
- Style anchor: "cinematic keynote look, deep navy #0B1220, soft volumetric
  glow, subtle reflective floor grid, generous clean center, no devices, no
  text."
- Reuse approved panels as reference images for later image-gen assets (same
  lighting, same palette) via the "extract parts" workflow in Stage 2.
...
| Decorative animation sprites (smoke, embers, glow orbs, sparkles, leaves, generic icons) | image-gen, transparent background | Composite + animate transforms; never product identity |
...
- Sprite (transparent for animation): "single <subject> (ember / smoke puff /
  glow orb / sparkle / leaf) isolated on pure solid black background, soft
  neon glow, high contrast, one element only, no text, no logos". Key out
  the solid background in post if the model returns one. For sprite atlases,
  frame sequences, chroma-key #00ff00, slicing and manifests, follow the full
  playbook in references/sprites.md.
...
- [ ] Outline + storyboard panels approved (image-gen plan phase) before final copy
